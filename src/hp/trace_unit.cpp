...
void TraceUnit::sampleSubTrace(Scene * scene, TraceUnit::unit_insert_f insert_f) {
    ...
    new_strength = strength.cwiseProduct(material->specular);
    if(new_strength.norm() > MIN_STRENGTH) {
        // int samples = depth == 0 ? int(std::sqrt(std::max(1, 100 - material->specular_exp))) : 1;
        // int samples = SPECULAR_SAMPLE;
        int samples = int(Number(SPECULAR_SAMPLE) * strength.norm());
        TraceUnit unit;
        unit.orig_id = this->orig_id;
        unit.depth = this->depth + 1;
    ...
    // diffuse
    // int samples = std::max(DIFFUSE_SAMPLE / (depth + 1) / (depth + 1), 1);
    int samples = int(Number(DIFFUSE_SAMPLE) * strength.norm());
    // int samples = DIFFUSE_SAMPLE;
    for(int i = 0 ; i < samples ; i += 1) {
        Vec p(RAND_F(), RAND_F(), RAND_F()); p.normalize();
        Number dot_normal = p.dot(normal);
        ...
        if(dot_normal < 0) {
            dot_normal = -dot_normal;
            if(reflection_dir.dot(normal) > 0)
                p = -p;
        }
        ...
        new_strength = strength.cwiseProduct(material->diffuse * dot_normal) / samples;
        if(new_strength.norm() > MIN_STRENGTH) {
            TraceUnit unit;
            unit.orig_id = this->orig_id;
            unit.depth = this->depth + 1;
